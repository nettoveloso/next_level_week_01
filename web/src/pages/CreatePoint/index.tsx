...
const CreatePoint = () => {
  ...
  const [selectItems, setSelectIntems] = useState<number[]>([]);
  ...
  function handleSelectItem(id: number){
    ...
    if(alreadySelected >= 0){ 
      const filterdItems = selectItems.filter(item=> item !== id);
      setSelectIntems(filterdItems);
    }else{
      setSelectIntems([ ...selectItems, id]);
    }   
  }
  ...
  async function handleSubmit(event: FormEvent){
    ...
    const {name, email, whatassp} = formData;
    const uf = selectedUf;
